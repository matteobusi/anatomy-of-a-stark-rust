...
impl MerkleTree {
    ...
    pub fn new (leaves : Vec<[u8; HL]>) -> MerkleTree {
        assert!(leaves.len() & (leaves.len()-1) == 0); // The number of leaves must be a power of two

        if leaves.len() == 1 {
            MerkleTree::Leaf { data : leaves[0] }
        } else {
            let (l, r) = (MerkleTree::new(leaves[..leaves.len()/2].to_vec()), MerkleTree::new(leaves[leaves.len()/2..].to_vec()));
    ...
    pub fn verify (root : [u8; HL], index : usize, proof : Vec<[u8; HL]>, leaf : [u8; HL]) -> bool {
        ...
        if proof.len() == 1 && index == 0 {
            ...
            MerkleTree::verify(root, index/2, proof[1..].to_vec(), MerkleTree::hash([leaf, proof[0]].concat()))
        } else {
            MerkleTree::verify(root, index/2, proof[1..].to_vec(), MerkleTree::hash([proof[0], leaf].concat()))
        }
    }
